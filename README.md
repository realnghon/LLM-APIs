...
- 按月自动归档历史记录，统计信息涵盖所有归档数据，无记录数量限制
...
使用记录以 NDJSON 格式追加式存储，按月自动归档：

- **当前月数据**：`apis-data/state.usage.ndjson`
- **历史归档**：`apis-data/state.usage-YYYY-MM.ndjson`（例如 `state.usage-2024-01.ndjson`）

每次记录新请求时，系统会检查是否需要归档（默认每 24 小时检查一次）。属于上个月或更早的记录会自动移到对应月份的归档文件中。

**统计数据计算**：
- 累计统计（cumulative）会聚合所有归档文件和当前月数据
- 每日趋势、账号统计、模型统计都包含完整历史
- 无记录数量限制，所有历史数据都会保留

**查询时间范围**：
- API 支持 `from` 和 `to` 参数按时间过滤记录
- 管理后台查询会根据过滤条件按需加载相应的归档文件

**清空记录**：
- 在管理后台执行清空操作会删除当前月数据和所有历史归档文件

归档文件按时间顺序命名，便于手动备份或迁移。如需迁移历史数据，只需复制对应的 `.ndjson` 文件到新环境的 `apis-data/` 目录即可。
